...
-   PostHog: website and mobile product analytics, including page views, navigation patterns, browser metadata, mobile feature events, account-linked web and mobile analytics after sign-in, and billing lifecycle events for checkout, subscriptions, and invoices, including plan, subscription status, amount and currency, and Stripe customer, subscription, and invoice identifiers. In a separate CodeRouter analytics project, model usage is recorded only as token counts, the model and provider category needed for pricing, a versioned API-equivalent cost estimate, and a keyed pseudonymous team scope. CodeRouter usage events do not include raw team IDs, prompts, model output, member identities, emails, account labels, provider-account identifiers, or credentials. We also record a submitted waitlist email so we can notify you.

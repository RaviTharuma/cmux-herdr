...
pub enum UpdateError {
    ...
    UpdateFailed { status: i32, restored: bool },
    InvalidReplacement { reason: String, restored: bool },
}
...
impl fmt::Display for UpdateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Command { program, status, stderr } => {
                write!(f, "{program} exited with status {status}")?;
                if !stderr.trim().is_empty() { write!(f, ": {}", stderr.trim())?; }
                Ok(())
            }
            Self::UpdateFailed { status, restored } => write!(
                f,
                "herdr update failed with status {status}{}",
                if *restored { "; previous binary restored" } else { "" },
            ),
...
enum TomlStringState { Normal, MultilineBasic, MultilineLiteral }
...
fn managed_block(text: &str) -> Result<Option<ManagedBlock>> {
    ...
    for (line_index, segment) in text.split_inclusive('\n').enumerate() {
        ...
        if is_comment && content == BLOCK_START { starts.push(lines.len() - 1); }
        if is_comment && content == BLOCK_END { ends.push(lines.len() - 1); }
        if is_comment
            && content.contains("cmux-herdr:herdr-auto-update:")
            && content != BLOCK_START
            && content != BLOCK_END
        {
            return Err(UpdateError::Config("malformed cmux-herdr auto-update marker".into()));
        }
        offset = end;
    }
    if starts.is_empty() && ends.is_empty() { return Ok(None); }
    if starts.len() != 1 || ends.len() != 1 || starts[0] >= ends[0] {
        return Err(UpdateError::Config("malformed cmux-herdr auto-update marker block".into()));
    }
    ...
    if active_header.and_then(|line| line.split('#').next()).map(str::trim) != Some("[update]") {
        return Err(UpdateError::Config("managed block is not inside [update]".into()));
    }
    ...
    let table = parsed.get("update").and_then(Item::as_table)
    ...
    Ok(Some(ManagedBlock { start: lines[start_line].0, end: lines[end_line].1 }))
}

fn remove_managed_block(text: &str) -> Result<String> {
    let Some(block) = managed_block(text)? else { return Ok(text.to_owned()); };
...
pub fn install_settings(text: &str, channel: &str, manifest_url: &str) -> Result<String> {
    ...
    if channel.trim().is_empty() {
        return Err(UpdateError::Config("update channel must not be empty".into()));
    }
    ...
    let table = document["update"].as_table_mut().expect("table established above");
    if let Some(existing) = table.get("channel") {
        if existing.as_str() != Some(channel) {
            return Err(UpdateError::Config(format!(
                ...
                existing.as_str().map_or_else(|| existing.to_string(), str::to_owned),
            )));
        }
    }
    ...
    if !add_channel && !add_manifest { return Ok(clean); }
    ...
    table.key_mut(inserted[0]).expect("inserted key").leaf_decor_mut().set_prefix(format!("{BLOCK_START}\n"));
    table[*inserted.last().expect("non-empty")].as_value_mut().expect("inserted value").decor_mut().set_suffix(format!("\n{BLOCK_END}"));
...
pub enum FileChange { Changed, Unchanged }
...
where F: FnOnce(&str) -> Result<String> {
    ...
    if updated == original { return Ok(FileChange::Unchanged); }
    let mode = fs::metadata(path).map(|m| m.permissions().mode() & 0o7777).unwrap_or(0o600);
    atomic_write(path, updated.as_bytes(), mode)?;
    Ok(FileChange::Changed)
}

fn ensure_parent(path: &Path) -> Result<&Path> {
    let parent = path.parent().ok_or_else(|| UpdateError::Config(format!("{} has no parent directory", path.display())))?;
...
fn atomic_write(path: &Path, bytes: &[u8], mode: u32) -> Result<()> {
    ...
    let mut temporary = tempfile::Builder::new()
        .prefix(&format!(".{}.", path.file_name().unwrap_or_else(|| OsStr::new("update")).to_string_lossy()))
        .tempfile_in(parent)
        .map_err(|error| UpdateError::io(format!("create temporary file for {}", path.display()), error))?;
    temporary.write_all(bytes).map_err(|error| UpdateError::io(format!("write {}", path.display()), error))?;
    temporary.flush().map_err(|error| UpdateError::io(format!("flush {}", path.display()), error))?;
    temporary.as_file().sync_all().map_err(|error| UpdateError::io(format!("sync {}", path.display()), error))?;
    ...
    temporary.persist(path).map_err(|error| UpdateError::io(format!("replace {}", path.display()), error.error))?;
...
fn atomic_copy(source: &Path, destination: &Path) -> Result<()> {
    let bytes = fs::read(source).map_err(|error| UpdateError::io(format!("read {}", source.display()), error))?;
    let mode = fs::metadata(source).map_err(|error| UpdateError::io(format!("stat {}", source.display()), error))?.permissions().mode() & 0o7777;
...
pub trait ProcessLiveness { fn is_alive(&self, pid: u32) -> bool; }
...
impl ProcessLiveness for SystemLiveness {
    fn is_alive(&self, pid: u32) -> bool {
        let Some(pid) = i32::try_from(pid).ok().and_then(rustix::process::Pid::from_raw) else { return false; };
...
#[derive(Debug)]
pub struct UpdateLock { path: PathBuf, pid: u32, _advisory: File }

impl Drop for UpdateLock {
    fn drop(&mut self) {
        let owned = fs::read_to_string(self.path.join("pid"))
            .ok().and_then(|text| text.trim().parse::<u32>().ok()) == Some(self.pid);
        if owned {
...
#[derive(Debug)]
pub enum LockAttempt { Acquired(UpdateLock), Busy { pid: u32 } }

pub fn acquire_update_lock(state_root: &Path, liveness: &dyn ProcessLiveness) -> Result<LockAttempt> {
    fs::create_dir_all(state_root).map_err(|error| UpdateError::io(format!("create {}", state_root.display()), error))?;
    let advisory_path = state_root.join("herdr-auto-update.guard");
    let advisory = OpenOptions::new().read(true).write(true).create(true).open(&advisory_path)
    ...
    if let Err(error) = rustix::fs::flock(&advisory, rustix::fs::FlockOperation::NonBlockingLockExclusive) {
        if error == rustix::io::Errno::AGAIN || error == rustix::io::Errno::WOULDBLOCK {
            let path = state_root.join(LOCK_NAME);
            let owner = fs::symlink_metadata(&path).ok().filter(|metadata| metadata.file_type().is_dir())
                .and_then(|_| fs::read_to_string(path.join("pid")).ok())
                .and_then(|text| text.trim().parse().ok()).unwrap_or(0);
    ...
    match fs::create_dir(&path) {
        ...
        Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
            ...
            if !metadata.file_type().is_dir() {
                return Err(UpdateError::Config(format!("update lock is not a directory: {}", path.display())));
            }
            let owner = fs::read_to_string(path.join("pid")).ok().and_then(|text| text.trim().parse::<u32>().ok());
            if let Some(owner) = owner {
                if liveness.is_alive(owner) { return Ok(LockAttempt::Busy { pid: owner }); }
            }
            let stale = state_root.join(format!("{LOCK_NAME}.stale.{pid}"));
            fs::rename(&path, &stale).map_err(|error| UpdateError::io("take over stale update lock", error))?;
            let entries = fs::read_dir(&stale).map_err(|error| UpdateError::io("inspect stale update lock", error))?
                .collect::<std::result::Result<Vec<_>, _>>().map_err(|error| UpdateError::io("inspect stale update lock", error))?;
            if entries.iter().any(|entry| entry.file_name() != OsStr::new("pid")) {
                return Err(UpdateError::Config(format!("stale update lock contains unexpected files: {}", stale.display())));
            }
            if stale.join("pid").exists() { fs::remove_file(stale.join("pid")).map_err(|error| UpdateError::io("remove stale lock owner", error))?; }
            fs::remove_dir(&stale).map_err(|error| UpdateError::io("remove stale update lock", error))?;
            match fs::create_dir(&path) {
                Ok(()) => {}
                Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
                    let owner = fs::read_to_string(path.join("pid")).ok().and_then(|text| text.trim().parse().ok()).unwrap_or(0);
                    return Ok(LockAttempt::Busy { pid: owner });
                }
    ...
    Ok(LockAttempt::Acquired(UpdateLock { path, pid, _advisory: advisory }))
}

#[derive(Debug, Clone)]
pub struct UpdateRequest { pub herdr_binary: PathBuf, pub state_root: PathBuf, pub backup_limit: usize }

impl UpdateRequest {
    pub fn new(herdr_binary: PathBuf, state_root: PathBuf) -> Self {
        Self { herdr_binary, state_root, backup_limit: BACKUP_LIMIT }
    }
}
...
pub enum UpdateOutcome {
    Busy { pid: u32 },
    Current { version: String, digest: String },
    Updated { from_version: String, to_version: String, digest: String, backup: PathBuf },
}

fn command_required(runner: &dyn CommandRunner, program: &Path, args: &[&str]) -> Result<CommandOutput> {
    let args = args.iter().map(|arg| (*arg).to_owned()).collect::<Vec<_>>();
    let output = runner.run(program, &args).map_err(|error| UpdateError::io(format!("run {}", program.display()), error))?;
    if output.status != 0 {
        return Err(UpdateError::Command { program: program.display().to_string(), status: output.status, stderr: output.stderr });
    }
    Ok(output)
}

fn sha256(path: &Path) -> Result<String> {
    let mut file = File::open(path).map_err(|error| UpdateError::io(format!("open {}", path.display()), error))?;
    let mut digest = Sha256::new();
    let mut buffer = [0_u8; 64 * 1024];
    loop {
        let count = file.read(&mut buffer).map_err(|error| UpdateError::io(format!("read {}", path.display()), error))?;
        if count == 0 { break; }
...
fn restore_binary(backup: &Path, binary: &Path) -> Result<()> { atomic_copy(backup, binary) }

fn backup_name(version: &str) -> String {
    let safe = version.chars().map(|character| if character.is_ascii_alphanumeric() || "._-".contains(character) { character } else { '_' }).collect::<String>();
...
impl StagedPrune {
    fn rollback(self) -> Result<()> {
        ...
        for (original, staged) in self.moved.iter().rev() {
            if let Err(error) = fs::rename(staged, original) { failures.push(error.to_string()); }
        }
        if failures.is_empty() { Ok(()) }
        else { Err(UpdateError::Config(format!("backup-prune rollback errors: {}", failures.join("; ")))) }
    }
}

fn stage_backup_prune(directory: &Path, retained: &Path, limit: usize) -> Result<Option<StagedPrune>> {
    let mut backups = fs::read_dir(directory).map_err(|error| UpdateError::io(format!("read {}", directory.display()), error))?
        .filter_map(|entry| entry.ok())
        .filter(|entry| entry.file_name().to_string_lossy().starts_with("herdr-") && entry.path() != retained)
        .collect::<Vec<_>>();
    backups.sort_by_key(|entry| entry.metadata().and_then(|metadata| metadata.modified()).unwrap_or(UNIX_EPOCH));
    let remove_count = (backups.len() + 1).saturating_sub(limit.max(1));
    if remove_count == 0 { return Ok(None); }
    let staging = tempfile::Builder::new().prefix(".herdr-prune.").tempdir_in(directory)
    ...
    for entry in backups.into_iter().take(remove_count) {
        ...
        if let Err(error) = fs::rename(&original, &staged) {
            let original_error = UpdateError::io(format!("stage backup prune for {}", original.display()), error);
            let mut restore_failures = Vec::new();
            for (old, temporary) in moved.iter().rev() {
                if let Err(error) = fs::rename(temporary, old) { restore_failures.push(error.to_string()); }
            }
            if restore_failures.is_empty() { return Err(original_error); }
            return Err(UpdateError::Config(format!("{original_error}; backup-prune rollback errors: {}", restore_failures.join("; "))));
        }
        moved.push((original, staged));
    }
    Ok(Some(StagedPrune { _directory: staging, moved }))
}

#[derive(Serialize)]
struct InstalledRecord<'a> { version: &'a str, sha256: &'a str }

pub fn run_update(request: &UpdateRequest, runner: &dyn CommandRunner, liveness: &dyn ProcessLiveness) -> Result<UpdateOutcome> {
    ...
    let binary = fs::canonicalize(&request.herdr_binary)
        .map_err(|error| UpdateError::io(format!("resolve {}", request.herdr_binary.display()), error))?;
    let before_version = command_required(runner, &binary, &["--version"])?.stdout.trim().to_owned();
    let before_digest = sha256(&binary)?;
    let backup_dir = request.state_root.join("herdr-backups");
    fs::create_dir_all(&backup_dir).map_err(|error| UpdateError::io(format!("create {}", backup_dir.display()), error))?;
    let record_path = request.state_root.join("herdr-update.json");
    let record_snapshot = FileSnapshot::capture(record_path.clone())?;
    let temporary = tempfile::Builder::new().prefix(".herdr-backup.").tempfile_in(&backup_dir)
    ...
    let update = match runner.run(&binary, &update_args) {
        ...
        Err(error) => {
            ...
            if changed { restore_binary(temporary.path(), &binary)?; }
            return Err(UpdateError::io(
                if changed { "run herdr update --handoff (previous binary restored)" } else { "run herdr update --handoff" },
                error,
            ));
        }
    };
    ...
    if update.status != 0 {
        ...
        if changed { restore_binary(temporary.path(), &binary)?; }
        return Err(UpdateError::UpdateFailed { status: update.status, restored: changed });
    }
    if after_digest.as_deref() == Some(before_digest.as_str()) {
        return Ok(UpdateOutcome::Current { version: before_version, digest: before_digest });
    }
    let Some(after_digest) = after_digest else {
        restore_binary(temporary.path(), &binary)?;
        return Err(UpdateError::InvalidReplacement { reason: "updated binary is missing".into(), restored: true });
    };
    let after_version = match command_required(runner, &binary, &["--version"]) {
        ...
        Ok(_) => {
            restore_binary(temporary.path(), &binary)?;
            return Err(UpdateError::InvalidReplacement { reason: "version check returned no version".into(), restored: true });
        }
        Err(error) => {
            restore_binary(temporary.path(), &binary)?;
            return Err(UpdateError::InvalidReplacement { reason: error.to_string(), restored: true });
        }
    };
    let record = serde_json::to_vec_pretty(&InstalledRecord { version: &after_version, sha256: &after_digest })
    ...
    if let Err(error) = temporary.persist(&backup) {
        ...
        return Err(UpdateError::io(format!("retain {}", backup.display()), error.error));
    }
    ...
    if let Err(error) = atomic_write(&record_path, &record, 0o600) {
        ...
        if let Some(staged) = staged_prune {
            if let Err(restore_error) = staged.rollback() { recovery.push(restore_error.to_string()); }
        }
        if let Err(restore_error) = record_snapshot.restore() { recovery.push(restore_error.to_string()); }
        if let Err(restore_error) = restore_binary(&backup, &binary) { recovery.push(restore_error.to_string()); }
        if let Err(remove_error) = fs::remove_file(&backup) { recovery.push(remove_error.to_string()); }
        if recovery.is_empty() { return Err(error); }
        return Err(UpdateError::Config(format!("{error}; update rollback errors: {}", recovery.join("; "))));
    }
    drop(staged_prune);
    Ok(UpdateOutcome::Updated { from_version: before_version, to_version: after_version, digest: after_digest, backup })
}

/// Resolve an override or PATH entry without invoking a shell.
pub fn resolve_executable(override_path: Option<&Path>, path: Option<&OsStr>, name: &str) -> Result<PathBuf> {
    if let Some(path) = override_path {
        return fs::canonicalize(path).map_err(|error| UpdateError::io(format!("resolve {}", path.display()), error));
    }
    let path = path.ok_or_else(|| UpdateError::Config(format!("{name} not found on PATH")))?;
    for directory in std::env::split_paths(path) {
        let candidate = directory.join(name);
        if fs::metadata(&candidate).map(|metadata| metadata.is_file() && metadata.permissions().mode() & 0o111 != 0).unwrap_or(false) {
            return fs::canonicalize(&candidate).map_err(|error| UpdateError::io(format!("resolve {}", candidate.display()), error));
        }
    }
    Err(UpdateError::Config(format!("{name} not found on PATH")))
}
pub fn resolve_herdr_path(explicit: Option<&Path>, environment: Option<&Path>, path: Option<&OsStr>) -> Result<PathBuf> {
    resolve_executable(explicit.or(environment), path, "herdr")
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ServiceManager { Launchd { domain: String }, Systemd }
...
fn absolute_xdg(name: &str, fallback: PathBuf) -> PathBuf {
    std::env::var_os(name).map(PathBuf::from).filter(|path| path.is_absolute()).unwrap_or(fallback)
}

impl ServicePaths {
    pub fn discover(manager: &ServiceManager, herdr_override: Option<&Path>) -> Result<Self> {
        let home = std::env::var_os("HOME").map(PathBuf::from).filter(|path| path.is_absolute())
        ...
        let source_binary = std::env::current_exe().and_then(fs::canonicalize)
            .map_err(|error| UpdateError::io("resolve cmux-herdr executable", error))?;
        let environment_herdr = std::env::var_os("HERDR_BIN").filter(|value| !value.is_empty()).map(PathBuf::from);
        let herdr_binary = resolve_herdr_path(herdr_override, environment_herdr.as_deref(), std::env::var_os("PATH").as_deref())?;
        Ok(Self {
            home,
    ...
    pub fn runtime_binary(&self) -> PathBuf { self.data_root.join("bin").join(RUNTIME_NAME) }
    pub fn launchd_plist(&self) -> PathBuf { self.definition_dir.join(format!("{LABEL}.plist")) }
    pub fn systemd_service(&self) -> PathBuf { self.definition_dir.join(format!("{LABEL}.service")) }
    pub fn systemd_timer(&self) -> PathBuf { self.definition_dir.join(format!("{LABEL}.timer")) }
}
...
impl InstallRequest {
    pub fn new(manager: ServiceManager, paths: ServicePaths, channel: String, manifest_url: String) -> Self {
        Self { manager, paths, channel, manifest_url }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UninstallResult { pub config: FileChange, pub removed: Vec<PathBuf>, pub command_warnings: Vec<String> }
...
pub fn render_launchd(paths: &ServicePaths, _domain: &str) -> String {
    ...
    let state_home = xml_escape(&paths.state_root.parent().unwrap_or(&paths.state_root).display().to_string());
    let config_home = xml_escape(&paths.config_path.parent().and_then(Path::parent).unwrap_or(&paths.home).display().to_string());
...
fn systemd_quote(path: &Path) -> String { systemd_quote_text(&path.display().to_string(), true) }

pub fn render_systemd(paths: &ServicePaths) -> (String, String) {
    let home = systemd_quote_text(&format!("HOME={}", paths.home.display()), false);
    let state_home = systemd_quote_text(&format!("XDG_STATE_HOME={}", paths.state_root.parent().unwrap_or(&paths.state_root).display()), false);
    let config_home = systemd_quote_text(&format!("XDG_CONFIG_HOME={}", paths.config_path.parent().and_then(Path::parent).unwrap_or(&paths.home).display()), false);
    let service = format!(
...
#[derive(Clone)]
struct FileSnapshot { path: PathBuf, bytes: Option<Vec<u8>>, mode: u32 }

impl FileSnapshot {
    fn capture(path: PathBuf) -> Result<Self> {
        match fs::read(&path) {
            Ok(bytes) => {
                let mode = fs::metadata(&path).map_err(|error| UpdateError::io(format!("stat {}", path.display()), error))?.permissions().mode() & 0o7777;
                Ok(Self { path, bytes: Some(bytes), mode })
            }
            Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(Self { path, bytes: None, mode: 0o600 }),
            Err(error) => Err(UpdateError::io(format!("read {}", path.display()), error)),
        }
    }

    fn restore(&self) -> Result<()> {
        if let Some(bytes) = &self.bytes { atomic_write(&self.path, bytes, self.mode) }
        else {
            match fs::remove_file(&self.path) {
                Ok(()) => Ok(()),
                Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(()),
                Err(error) => Err(UpdateError::io(format!("remove {}", self.path.display()), error)),
            }
        }
    }
}

fn run_status(runner: &dyn CommandRunner, program: &str, args: &[&str], required: bool) -> Result<Option<CommandOutput>> {
    ...
    match runner.run(Path::new(program), &owned) {
        ...
        Ok(output) => Err(UpdateError::Command { program: program.into(), status: output.status, stderr: output.stderr }),
        Err(error) if error.kind() == io::ErrorKind::NotFound && !required => Ok(None),
        Err(error) => Err(UpdateError::io(format!("run {program}"), error)),
    }
}

fn probe_status(runner: &dyn CommandRunner, program: &str, args: &[&str]) -> Result<(bool, String)> {
    let owned = args.iter().map(|arg| (*arg).to_owned()).collect::<Vec<_>>();
    let output = runner.run(Path::new(program), &owned)
...
fn rollback(snapshots: &[FileSnapshot]) -> Result<()> {
    ...
    for snapshot in snapshots.iter().rev() {
        if let Err(error) = snapshot.restore() { failures.push(error.to_string()); }
    }
    if failures.is_empty() { Ok(()) }
    else { Err(UpdateError::Config(format!("rollback errors: {}", failures.join("; ")))) }
}

pub fn install_service(request: &InstallRequest, runner: &dyn CommandRunner) -> Result<InstallResult> {
    let paths = &request.paths;
    let defaults = command_required(runner, &paths.herdr_binary, &["--default-config"])?;
    if !defaults.stdout.lines().any(|line| line.trim_start().starts_with("# manifest_url = ")) {
        return Err(UpdateError::Config("installed Herdr does not support update.manifest_url".into()));
    }
    ...
    let prior_state = match &request.manager {
        ServiceManager::Launchd { domain } => {
            ...
            if loaded && !paths.launchd_plist().exists() {
                return Err(UpdateError::Config(format!("launchd label {LABEL} is loaded from an unmanaged definition")));
            }
            let disabled_output = run_status(runner, "launchctl", &["print-disabled", domain], true)?
                .expect("required launchctl command returned output").stdout;
            let disabled = disabled_output.lines().any(|line| line.contains(LABEL) && line.contains("=> true"));
            PriorServiceState::Launchd { loaded, enabled: !disabled }
        }
        ServiceManager::Systemd => {
            run_status(runner, "systemctl", &["--user", "show-environment"], true)?;
            let timer = format!("{LABEL}.timer");
            let (enabled, _) = probe_status(runner, "systemctl", &["--user", "is-enabled", &timer])?;
    ...
    let snapshots = targets.into_iter().map(FileSnapshot::capture).collect::<Result<Vec<_>>>()?;
    let attempt = (|| {
        let config = update_file(&paths.config_path, |text| install_settings(text, &request.channel, &request.manifest_url))?;
        if let Err(error) = command_required(runner, &paths.herdr_binary, &["config", "check"]) {
            return Err(UpdateError::Config(format!("managed update settings failed Herdr validation: {error}")));
        }
        atomic_copy(&paths.source_binary, &paths.runtime_binary())?;
        match &request.manager {
            ServiceManager::Launchd { domain } => {
                atomic_write(&paths.launchd_plist(), render_launchd(paths, domain).as_bytes(), 0o644)?;
            ...
            ServiceManager::Systemd => {
                ...
                run_status(runner, "systemd-analyze", &["verify", &service_path, &timer_path], false)?;
                run_status(runner, "systemctl", &["--user", "daemon-reload"], true)?;
                let timer_name = format!("{LABEL}.timer");
                run_status(runner, "systemctl", &["--user", "enable", "--now", &timer_name], true)?;
                let service_name = format!("{LABEL}.service");
                run_status(runner, "systemctl", &["--user", "start", &service_name], true)?;
            }
        }
        Ok(config)
    })();
    let config = match attempt {
        ...
        Err(error) => {
            match &request.manager {
                ...
                ServiceManager::Systemd => {
                    let timer = format!("{LABEL}.timer");
                    let _ = run_status(runner, "systemctl", &["--user", "disable", "--now", &timer], false);
            ...
            if let Err(rollback_error) = rollback(&snapshots) {
                return Err(UpdateError::Config(format!("{error}; rollback failed: {rollback_error}")));
            }
            let manager_restore = (|| -> Result<()> {
                match (prior_state, &request.manager) {
                    (PriorServiceState::Launchd { loaded, enabled }, ServiceManager::Launchd { domain }) => {
                        let plist = paths.launchd_plist().display().to_string();
                        if loaded {
                            run_status(runner, "launchctl", &["bootstrap", domain, &plist], true)?;
                        }
                    ...
                    (PriorServiceState::Systemd { enabled, active }, ServiceManager::Systemd) => {
                        ...
                        run_status(runner, "systemctl", &["--user", enable_operation, &timer], true)?;
                        let active_operation = if active { "start" } else { "stop" };
                        run_status(runner, "systemctl", &["--user", active_operation, &timer], true)?;
                    }
                    _ => return Err(UpdateError::Config("service manager changed during install".into())),
                }
                Ok(())
            })();
            if let Err(restore_error) = manager_restore {
                return Err(UpdateError::Config(format!("{error}; service rollback failed: {restore_error}")));
            }
            return Err(error);
        }
    };
    Ok(InstallResult {
        ...
        definitions,
        uninstall_command: vec![paths.runtime_binary().display().to_string(), "update-service".into(), "uninstall".into()],
    })
}

fn remove_existing(path: &Path, removed: &mut Vec<PathBuf>) -> Result<()> {
    match fs::remove_file(path) {
        Ok(()) => { removed.push(path.to_path_buf()); Ok(()) }
        Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(UpdateError::io(format!("remove {}", path.display()), error)),
    }
}

fn best_effort(runner: &dyn CommandRunner, program: &str, args: &[&str], warnings: &mut Vec<String>) {
    if let Err(error) = run_status(runner, program, args, false) { warnings.push(error.to_string()); }
}

pub fn uninstall_service(manager: &ServiceManager, paths: &ServicePaths, runner: &dyn CommandRunner) -> Result<UninstallResult> {
    ...
    match manager {
        ServiceManager::Launchd { domain } => {
            ...
            let plist = paths.launchd_plist().display().to_string();
            best_effort(runner, "launchctl", &["bootout", domain, &plist], &mut warnings);
        }
        ServiceManager::Systemd => {
            let timer = format!("{LABEL}.timer");
            let service = format!("{LABEL}.service");
            best_effort(runner, "systemctl", &["--user", "disable", "--now", &timer], &mut warnings);
            best_effort(runner, "systemctl", &["--user", "stop", &service], &mut warnings);
        }
    }
    let config = if paths.config_path.exists() { update_file(&paths.config_path, uninstall_settings)? } else { FileChange::Unchanged };
    ...
    if matches!(manager, ServiceManager::Systemd) {
        best_effort(runner, "systemctl", &["--user", "daemon-reload"], &mut warnings);
        let service = format!("{LABEL}.service");
        best_effort(runner, "systemctl", &["--user", "reset-failed", &service], &mut warnings);
    }
    Ok(UninstallResult { config, removed, command_warnings: warnings })
}
...
mod tests {
    ...
    impl CommandRunner for FakeRunner {
        fn run(&self, program: &Path, args: &[String]) -> std::io::Result<CommandOutput> {
            ...
            let probe_miss = args.first().map(String::as_str) == Some("print")
                || args.iter().any(|arg| arg == "is-enabled" || arg == "is-active");
            let status = self.argument_failures.get(args).or_else(|| self.failures.get(&name)).copied()
            ...
            Ok(CommandOutput { status, stdout, stderr: String::new() })
        }
    }

    struct FixedLiveness(bool);
    impl ProcessLiveness for FixedLiveness {
        fn is_alive(&self, _pid: u32) -> bool { self.0 }
    }
        ...
        fn run(&self, _program: &Path, args: &[String]) -> std::io::Result<CommandOutput> {
            if args == ["--version"] {
                ...
                let stdout = if *calls == 0 {
                    ...
                    *calls += 1;
                    return Ok(CommandOutput { status: 1, stdout: String::new(), stderr: "bad replacement".into() });
                };
                *calls += 1;
                return Ok(CommandOutput { status: 0, stdout, stderr: String::new() });
            }
            ...
            Ok(CommandOutput { status: self.update_status, stdout: String::new(), stderr: String::new() })
        }
    }
    ...
    fn config_install_uninstall_preserves_comments_order_and_mode() {
        ...
        assert_eq!(update_file(&path, |text| install_settings(text, "preview", MANIFEST)).unwrap(), FileChange::Changed);
    ...
    fn config_install_is_idempotent_and_matching_values_stay_user_owned() {
        ...
        assert_eq!(install_settings(&original, "preview", MANIFEST).unwrap(), original);
        let managed = install_settings("[update]\nversion_check = true\n", "preview", MANIFEST).unwrap();
        assert_eq!(install_settings(&managed, "preview", MANIFEST).unwrap(), managed);
    }

    #[test]
    fn config_rejects_conflicts_non_https_and_malformed_markers() {
        assert!(install_settings("[update]\nchannel = \"stable\"\n", "preview", MANIFEST).unwrap_err().to_string().contains("channel"));
        assert!(install_settings("[update]\nmanifest_url = \"https://elsewhere.test/x\"\n", "preview", MANIFEST).unwrap_err().to_string().contains("manifest"));
        assert!(install_settings("", "preview", "http://example.com/x").unwrap_err().to_string().contains("https"));
        let malformed = format!("[update]\n{BLOCK_START}\nchannel = \"preview\"\n");
        assert!(uninstall_settings(&malformed).unwrap_err().to_string().contains("marker"));
        let foreign = format!("[update]\n{BLOCK_START}\nchannel = \"preview\"\nuser_key = true\n{BLOCK_END}\n");
        assert!(uninstall_settings(&foreign).unwrap_err().to_string().contains("managed block"));
    }
    ...
    fn herdr_environment_override_precedes_path_lookup() {
        ...
        let resolved = resolve_herdr_path(None, Some(&override_path), Some(dir.path().as_os_str())).unwrap();
    ...
    fn update_file_is_atomic_and_noop_aware() {
        ...
        assert_eq!(update_file(&path, |_| Ok("[update]\n".into())).unwrap(), FileChange::Changed);
        assert_eq!(update_file(&path, |text| Ok(text.into())).unwrap(), FileChange::Unchanged);
    ...
    fn live_lock_is_busy_and_left_untouched() {
        ...
        assert!(matches!(acquire_update_lock(dir.path(), &FixedLiveness(true)).unwrap(), LockAttempt::Busy { pid: 42 }));
    ...
    fn advisory_lock_prevents_takeover_during_owner_liveness_race() {
        ...
        assert!(matches!(acquire_update_lock(dir.path(), &FixedLiveness(false)).unwrap(), LockAttempt::Busy { .. }));
    }
    ...
    fn lock_symlink_is_rejected_without_touching_target() {
        ...
        assert_eq!(fs::read_to_string(target.join("pid")).unwrap(), "99999999\n");
    }
    ...
    fn unchanged_update_is_noop_without_backup() {
        ...
        let runner = ScriptedUpdateRunner { binary: binary.clone(), replacement: None, update_status: 0, after_version: None, before_version: "herdr 1.0\n".into(), version_calls: RefCell::new(0) };
        let outcome = run_update(&UpdateRequest::new(binary, dir.path().join("state")), &runner, &FixedLiveness(false)).unwrap();
        assert!(matches!(outcome, UpdateOutcome::Current { .. }));
        assert_eq!(fs::read_dir(dir.path().join("state/herdr-backups")).unwrap().count(), 0);
    }
    ...
    fn failed_or_invalid_replacement_restores_bytes_and_mode() {
        for (status, after_version) in [(7, Some("herdr bad\n".into())), (0, None)] {
            ...
            fs::write(&binary, b"old binary").unwrap();
            fs::set_permissions(&binary, fs::Permissions::from_mode(0o750)).unwrap();
            let runner = ScriptedUpdateRunner { binary: binary.clone(), replacement: Some(b"replacement".to_vec()), update_status: status, after_version, before_version: "herdr 1.0\n".into(), version_calls: RefCell::new(0) };
            assert!(run_update(&UpdateRequest::new(binary.clone(), dir.path().join("state")), &runner, &FixedLiveness(false)).is_err());
    ...
    fn successful_update_records_digest_and_keeps_three_backups() {
        ...
        let runner = ScriptedUpdateRunner { binary: binary.clone(), replacement: Some(b"new binary".to_vec()), update_status: 0, after_version: Some("herdr 2.0\n".into()), before_version: "herdr 1.0\n".into(), version_calls: RefCell::new(0) };
        let outcome = run_update(&UpdateRequest::new(binary, state.clone()), &runner, &FixedLiveness(false)).unwrap();
        let UpdateOutcome::Updated { digest, backup, .. } = outcome else { panic!("expected update") };
        assert_eq!(digest.len(), 64);
        assert!(backup.exists());
        assert_eq!(fs::read_dir(state.join("herdr-backups")).unwrap().count(), 3);
    ...
    fn templates_render_real_paths_and_distinct_schedules() {
        ...
        assert!(launchd.contains(&xml_escape(&paths.state_root.parent().unwrap().display().to_string())));
        assert!(launchd.contains("XDG_CONFIG_HOME"));
        assert!(launchd.contains(&xml_escape(&paths.config_path.parent().unwrap().parent().unwrap().display().to_string())));
        let (service, timer) = render_systemd(&paths);
        ...
        assert!(service.contains(&paths.config_path.parent().unwrap().parent().unwrap().display().to_string()));
    ...
    fn prepare_install(paths: &ServicePaths) {
        ...
        fs::write(&paths.config_path, "[update]\nversion_check = true\n\n[ui]\nkeep = true\n").unwrap();
    ...
    fn launchd_install_and_uninstall_are_reversible() {
        ...
        let result = install_service(&InstallRequest::new(ServiceManager::Launchd { domain: "gui/501".into() }, paths.clone(), "preview".into(), MANIFEST.into()), &runner).unwrap();
        ...
        let removed = uninstall_service(&ServiceManager::Launchd { domain: "gui/501".into() }, &paths, &runner).unwrap();
    ...
    fn launchd_enable_failure_rolls_back_install() {
        ...
        runner.argument_failures.insert(vec!["enable".into(), format!("gui/501/{LABEL}")], 1);
        assert!(install_service(&InstallRequest::new(ServiceManager::Launchd { domain: "gui/501".into() }, paths.clone(), "preview".into(), MANIFEST.into()), &runner).is_err());
        assert_eq!(fs::read_to_string(&paths.config_path).unwrap(), original);
        assert!(!paths.launchd_plist().exists());
        assert!(!paths.runtime_binary().exists());
    }
    ...
    fn systemd_install_validates_then_uninstall_cleans_units() {
        ...
        prepare_install(&paths);
        let runner = FakeRunner::supporting();
        install_service(&InstallRequest::new(ServiceManager::Systemd, paths.clone(), "preview".into(), MANIFEST.into()), &runner).unwrap();
    ...
    fn failed_activation_rolls_back_config_runtime_and_units() {
        ...
        let original = fs::read_to_string(&paths.config_path).unwrap();
        let mut runner = FakeRunner::supporting();
        runner.argument_failures.insert(vec!["--user".into(), "enable".into(), "--now".into(), format!("{LABEL}.timer")], 5);
        assert!(install_service(&InstallRequest::new(ServiceManager::Systemd, paths.clone(), "preview".into(), MANIFEST.into()), &runner).is_err());
        let reloads = runner.calls.borrow().iter().filter(|(_, args)| args == &["--user", "daemon-reload"]).count();

...
- Official plugin-manager package (`cmux-plugin.toml` + `bin/cmux-herdr-sidebar`) + agent skill documenting the dual hierarchy.
...
5. **Install is the official cmux plugin manager, and only that.**
   Users run `cmux sidebar plugin install` / `use` / `update` / `remove`.
   Copying `sidebars/herdr.*` into `~/.config/cmux/sidebars` and running
   `cmux sidebar select|open herdr` is a legacy contrib fallback, not the
   product. `./scripts/install.sh` is contributor/dev only. There is no
   Homebrew formula or signed app bundle. See [RELEASE.md](./RELEASE.md).

...
fn pure_behavior_matches_python() {
    ...
    for entry in g["env_truthy"].as_array().unwrap() {
        ...
        assert_eq!(json!(handoff::env_truthy("HANDOFF_GOLDEN_FLAG")), entry["result"]);
    }

    for entry in g["lease_ttl_ms"].as_array().unwrap() {
        set_env(handoff::LEASE_TTL_ENV, &entry["value"]);
        assert_eq!(json!(handoff::lease_ttl_ms()), entry["result"], "TTL for {:?}", entry["value"]);
    }
    ...
    file.set_times(std::fs::FileTimes::new().set_modified(UNIX_EPOCH + Duration::from_secs(1_700_000_000))).unwrap();
    drop(file);
    for entry in g["parse_lease_text"].as_array().unwrap() {
        ...
        assert_eq!(lease_json(got), entry["result"], "parse case {}", entry["name"]);
    }
    ...
    for entry in g["freshness"].as_array().unwrap() {
        ...
        assert_eq!(
            json!(lease.is_fresh(Some(entry["now"].as_i64().unwrap()), Some(entry["ttl"].as_i64().unwrap()))),
            entry["result"]
        );
    }

    std::env::set_var("XDG_STATE_HOME", "/tmp/cmux-herdr-handoff-golden/xdg");
    std::env::set_var(handoff::NATIVE_STATE_ENV, "/tmp/cmux-herdr-handoff-golden/native");
